Notes = Backbone.Model.extend({
    ...
    initialize: function () {
        ...
        //GlobalEvent.listenTo("note-created", this.afterCreateNote);
        //GlobalEvent.listenTo("note-updated", this.afterUpdateNote);
        GlobalEvent.listenTo("user-info-fetched", this.setUserName);
    },

    connect: function (live) {
        var that = this;
        if (live) {
    ...
    deleteNote: function (note) {
        this.storage.deleteNote(note, this.get('events'));
    },

    addNote: function (content, tag, date) {
        var tagLowerCased = tag.toLowerCase().trim();
        var note = {id: content.hashCode(), content: content, type: tagLowerCased, date: date.toDate()}
