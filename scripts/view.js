TimeSliderView = Backbone.View.extend({
    ...
    login: function (e) {
        $(".open-banner .controls").hide();
        $(".open-banner .spinner").show();
        $(".floating-login").hide();
        var migrationNeeded = $(e.currentTarget).hasClass("dropbox");
        this.model.connect(true, migrationNeeded);
    ...
    showNewMarkerPopup: function () {
        ...
        newMarkerPopup.find("input.label").val("");
        newMarkerPopup.find("input.date").val(moment().format("DD/MM/YYYY"));
        newMarkerPopup.show();
        newMarkerPopup.find(".label").focus();
    },
